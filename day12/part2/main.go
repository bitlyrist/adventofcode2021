...
func traverse(paths map[string][]string, small_caves map[string]int, path []string, cave string) int {
	...
	if cannot_traverse(small_caves, cave) {
		return 0
	}
...
func can_traverse (caves map[string]int, cave string) bool {
	if is_upper(cave) {
		return true
	}
	if caves[cave] == 0 {
...
func cannot_traverse (caves map[string]int, cave string) bool {
	return !can_traverse(caves, cave)
}

func max_traverse(small_caves map[string]int) int {
	max := 0
	for _,trav := range small_caves {
		if trav > max {
			max = trav
		}
	}
