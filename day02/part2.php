...
foreach ($input as $i)  {
  ...
  $location['y'] += $directions[$i[0]]['x'] * $i[1] * $aim;
  
  // $location['y'] += $directions[$i[0]]['y'] * $i[1];
  // $location['y'] = $location['y'] < 0 ? 0 : $location['y'];
  
  $aim += $directions[$i[0]]['y'] * $i[1];
  printf("Current location is: [%d, %d] with aim %d\n", $location['x'], $location['y'], $aim);

}

printf("Final location is: [%d, %d] with aim %d\n", $location['x'], $location['y'], $aim);
